...
class GameEngine {
    ...
    damagePlayer(amount) {
        ...
        this.triggerShake(10); // Massive shake
    ...
    levelUp() {
        ...
        const choices = this.upgrades.getUpgrades();
        choices.forEach(choice => {
            const card = document.createElement('div');
            card.className = 'upgrade-card p-6 rounded-lg text-center';
            card.innerHTML = `
    ...
    update(dt, currentTime) {
        ...
        this.player.update(this.mouse.x, this.mouse.y, dt, this.mouse.isDown);
        ...
        for (let i = this.enemies.length - 1; i >= 0; i--) {
            ...
            if (e.markedForDeletion) {
                this.spawnParticles(e.x, e.y, 15, e.color, 2); // Explosion
                this.triggerShake(3);
                this.score += 10;
                document.getElementById('scoreDisplay').innerText = this.score;
